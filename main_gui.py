...
def export_results():
    ...
    with open("export.json", "w", encoding="utf-8") as file:  # Открываем файл "export.json" для записи с кодировкой "utf-8"
        json.dump(data, file, ensure_ascii=False, indent=4)  # Записываем данные в файл в формате JSON с отступами
    dpg.set_value("status", "Результаты экспортированы в файл export.json.")  # Устанавливаем сообщение о статусе экспорта
...
def show_about():
    ...
    with dpg.window(label="О программе", width=400, height=300, modal=True, tag="about_window"):  # Создание нового окна "О программе"
        ...
        dpg.add_text("Разработчик: Чайковская Елена Витальевна")  # Добавление текста "Разработчик: Чайковская Елена Витальевна"
...
def setup_fonts():
    with dpg.font_registry():  # Создание реестра шрифтов
        with dpg.font("C:/Windows/Fonts/Arial.ttf", 20) as default_font:  # Добавление шрифта Arial размером 20
...
def main_window():
    with dpg.window(label="Основное окно", width=800, height=600):  # Создаем основное окно
